...
test("handles document quality issues", async ({ page }) => {
  ...
  await expect(page.getByText("Place document on flat surface")).toBeVisible();
...
test("handles camera permissions denied", async ({ page }) => {
  ...
  await expect(page.getByText('2. Select "Allow" when prompted')).toBeVisible();
...
test("handles browser compatibility issues", async ({ page }) => {
  ...
  await expect(page.getByText("Try Chrome, Firefox, or Safari")).toBeVisible();

...
export const VIETNAMESE_TELCOS: Record<string, VietnameseTelco> = {
  VIETTEL: {
    name: 'Viettel',
    code: 'VIETTEL',
    prefixes: ['032', '033', '034', '035', '036', '037', '038', '039', '096', '097', '098', '086', '081', '082', '083', '084', '085'],
    otpLength: 4,
    maxAttempts: 3,
    resendCooldown: 60,
  ...
  MOBIFONE: {
    name: 'Mobifone',
    code: 'MOBIFONE',
    prefixes: ['090', '093', '070', '079', '0120', '0121', '0122', '0126', '0128', '089', '056', '058', '059'],
    otpLength: 6,
    maxAttempts: 3,
    resendCooldown: 90,
  ...
  VINAPHONE: {
    name: 'Vinaphone',
    code: 'VINAPHONE',
    prefixes: ['091', '094', '083', '084', '085', '081', '082', '088', '058', '086'],
    otpLength: 6,
    maxAttempts: 3,
    resendCooldown: 90,
    otpExpiry: 600, // 10 minutes
  ...
  VIETNAMOBILE: {
    name: 'Vietnamobile',
    code: 'VIETNAMOBILE',
    prefixes: ['092', '056', '058', '032', '033', '034', '035', '036', '037', '038', '039'],
    otpLength: 4,
    maxAttempts: 3,
    resendCooldown: 60,
    otpExpiry: 300, // 5 minutes
  ...
  GTEL: {
    name: 'Gmobile',
    code: 'GTEL',
    prefixes: ['099', '059', '032', '033', '034', '035', '036', '037', '038', '039'],
    otpLength: 4,
    maxAttempts: 3,
    resendCooldown: 60,
    otpExpiry: 300, // 5 minutes
 ...
 */
export const getTelcoByPhoneNumber = (phoneNumber: string): VietnameseTelco | null => {
 ...
 */
export const addNumberPortingRecord = (phoneNumber: string, newTelcoCode: string): void => {
  const cleanPhone = phoneNumber.replace(/\D/g, '');
  if (cleanPhone.length === 10 && cleanPhone.startsWith('0')) {
 ...
 */
export const getTelcoByPhoneNumberWithPorting = (phoneNumber: string): VietnameseTelco | null => {
  const telco = getTelcoByPhoneNumber(phoneNumber);
...
export const validateRegulatoryCompliance = (
  ...
  if (otpAttempts > VIETNAM_REGULATORY.OTP_REQUIREMENTS.MAX_OTP_PER_DAY_PER_USER) {
    violations.push(`Daily OTP limit exceeded: ${otpAttempts}/${VIETNAM_REGULATORY.OTP_REQUIREMENTS.MAX_OTP_PER_DAY_PER_USER}`);
  }
  ...
  if (lastOtpTime) {
    ...
    if (timeSinceLastOtp < 30000) { // 30 seconds
      violations.push('OTP requests too frequent - minimum 30 seconds required');
    }
  }

  // Validate phone number format for Vietnamese regulations
  if (!isValidVietnamesePhoneNumber(phoneNumber)) {
    violations.push('Phone number does not comply with Vietnamese numbering plan');
  }

  // Check for Vietnamese telco support
  const telco = getTelcoByPhoneNumberWithPorting(phoneNumber);
  if (!telco) {
    violations.push('Phone number not associated with licensed Vietnamese telecom provider');
  }

  // Recommendations
  if (otpAttempts > VIETNAM_REGULATORY.OTP_REQUIREMENTS.MAX_OTP_PER_DAY_PER_USER * 0.7) {
    recommendations.push('Approaching daily OTP limit - consider alternative authentication');
  }
 ...
 */
export const getComplianceReport = (phoneNumber: string): {
  phoneNumber: string;
  telco: string;
  supportsOTP: boolean;
  ...
  return {
    ...
    recommendations: [
      ...
      telco ? `Use ${telco.displayName} short code: ${telco.shortCode}` : 'No supported telco detected',
...
export const TELCO_ERROR_MESSAGES = {
  INVALID_PHONE: 'Số điện thoại không hợp lệ. Vui lòng nhập số điện thoại Việt Nam.',
  UNSUPPORTED_TELCO: 'Nhà mạng không được hỗ trợ. Vui lòng sử dụng số điện thoại của các nhà mạng được cấp phép tại Việt Nam.',
  INVALID_OTP_LENGTH: 'Mã OTP không đúng độ dài. Vui lòng kiểm tra lại.',
  OTP_EXPIRED: 'Mã OTP đã hết hạn. Vui lòng yêu cầu mã mới.',
  MAX_ATTEMPTS_EXCEEDED: 'Bạn đã nhập sai mã OTP quá số lần cho phép. Vui lòng thử lại sau.',
  ...
  COMPLIANCE_VIOLATION: 'Yêu cầu không tuân thủ quy định của Bộ Thông tin và Truyền thông.',
  DATA_PRIVACY_CONCERN: 'Quyền riêng tư dữ liệu không được đảm bảo. Vui lòng liên hệ hỗ trợ.',
  REGULATORY_RESTRICTION: 'Giới hạn theo quy định pháp luật Việt Nam.',
  PORTED_NUMBER_DETECTED: 'Số điện thoại đã chuyển mạng. Vui lòng xác thực với nhà mạng mới.',

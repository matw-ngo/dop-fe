 ...
 */
export function useTranslationMonitor(options: UseTranslationMonitorOptions = {}) {
  const {
   ...
   */
  const trackRequest = useCallback((key: string, ns?: string) => {
   ...
   */
  const useMonitoredTranslations = useCallback((ns?: string) => {
...
const TranslationMonitorContext = createContext<TranslationMonitorContextValue | null>(null);
...
export function TranslationMonitorProvider({
  ...
  const setOptions = useCallback((newOptions: Partial<UseTranslationMonitorOptions>) => {
...
export function useTranslationMonitorContext() {
  ...
  if (!context) {
    throw new Error('useTranslationMonitorContext must be used within TranslationMonitorProvider');
  }
  return context;
}
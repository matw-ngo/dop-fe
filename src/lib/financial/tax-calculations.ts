...
import { TaxCalculationResult, TaxCalculationParams } from '../financial-data/tax-brackets';
import { SOCIAL_INSURANCE_RATES, FAMILY_DEDUCTIONS, REGIONAL_MINIMUM_WAGE } from '../financial-data/vietnamese-financial-data';
...
export const calculateComprehensivePersonalTax = (
  ...
  const unemploymentInsurance = Math.min(
    params.grossMonthlyIncome * SOCIAL_INSURANCE_RATES.unemploymentInsuranceEmployee,
    maxUnemploymentInsuranceBase * SOCIAL_INSURANCE_RATES.unemploymentInsuranceEmployee
  );

  const totalInsurance = socialInsurance + healthInsurance + unemploymentInsurance;

  // Calculate family deductions
  const familyDeductionsAmount = FAMILY_DEDUCTIONS.taxpayerDeduction +
    (params.numberOfDependents * FAMILY_DEDUCTIONS.dependentDeduction);

  // Calculate taxable income
  const taxableIncome = Math.max(0, params.grossMonthlyIncome - totalInsurance - familyDeductionsAmount);
  ...
  const effectiveTaxRate = params.grossMonthlyIncome > 0 ? (incomeTax / params.grossMonthlyIncome) * 100 : 0;
  ...
  if (params.grossMonthlyIncome > 20000000 && params.numberOfDependents === 0) {
    taxOptimizationOpportunities.push('Consider registering family members as dependents if eligible');
  }

  if (socialInsurance < 2380000 && params.grossMonthlyIncome >= 29750000) {
    taxOptimizationOpportunities.push('Consider optimizing social insurance contributions');
  }

  if (effectiveTaxRate > 25) {
    taxOptimizationOpportunities.push('High tax rate detected - consider professional tax advice');
  }
  ...
  const percentileRank = Math.max(0, Math.min(100, (1 - (effectiveTaxRate - 5) / 25) * 100));
...
export const calculateCorporateTax = (
  ...
  const effectiveTaxRate = taxableIncome > 0 ? (corporateTax / taxableIncome) * 100 : 0;
  ...
  switch (type) {
    ...
    case 'import_export':
      if (details.productValue) {
        ...
        const taxAmount = details.productValue * ((importTaxRate + exportTaxRate) / 100);
...
export const analyzeCompensationTax = (
  ...
  const baseParams = { grossMonthlyIncome: baseSalary, numberOfDependents, region };
  ...
  const withBonusParams = { ...baseParams, grossMonthlyIncome: baseSalary + bonus };
  ...
  const withAllowancesParams = { ...baseParams, grossMonthlyIncome: baseSalary + allowances };
  const withAllowancesTax = calculateComprehensivePersonalTax(withAllowancesParams);
  ...
  if (bonus > 0) {
    const bonusTaxRate = (withBonusTax.incomeTax - baseSalaryTax.incomeTax) / bonus * 100;
    if (bonusTaxRate > 25) {
      recommendations.push('Consider converting bonus to allowances or performance-based compensation');
    }
  }

  if (allowances > 0) {
    const allowanceTaxRate = (withAllowancesTax.incomeTax - baseSalaryTax.incomeTax) / allowances * 100;
    if (allowanceTaxRate < 15) {
      mostEfficientStructure = 'allowances_heavy';
      recommendations.push('Focus on non-taxable allowances for better tax efficiency');
    }
  }

  const taxSavingsOpportunity = Math.max(0, totalCompensationTax.incomeTax - baseSalaryTax.incomeTax * 1.2);
...
export const generateTaxComplianceChecklist = (
  ...
  const checklist = {
    ...
    deadlines: [] as Array<{ type: string; deadline: string; frequency: string }>,
  ...
  switch (taxpayerType) {
    case 'individual':
      ...
      checklist.deadlines = [
        { type: 'Annual tax filing', deadline: 'March 31', frequency: 'Yearly' },
        { type: 'Quarterly tax payment', deadline: 'Last day of quarter', frequency: 'Quarterly' },
        { type: 'Tax registration', deadline: 'Within 10 days of starting work', frequency: 'Once' },
      ];
      ...
      if (annualIncome > 120000000) { // 120 triệu VND/year
    ...
    case 'corporate':
      ...
      checklist.deadlines = [
        { type: 'Monthly VAT filing', deadline: '20th of following month', frequency: 'Monthly' },
        { type: 'Quarterly CIT filing', deadline: '30th of following month', frequency: 'Quarterly' },
        { type: 'Annual CIT filing', deadline: 'April 30', frequency: 'Yearly' },
      ];
    ...
    case 'freelancer':
      ...
      checklist.deadlines = [
        { type: 'Quarterly tax filing', deadline: '20th of following month', frequency: 'Quarterly' },
        { type: 'Annual tax reconciliation', deadline: 'March 31', frequency: 'Yearly' },
      ];

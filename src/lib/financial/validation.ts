...
import { LoanCalculationParams, TaxCalculationParams } from './calculations';
 ...
 */
export const validateLoanCalculationParams = (params: LoanCalculationParams): ValidationResult => {
  ...
  if (score > 80) {
    recommendations.push('Thông tin khoản vay hợp lệ - có thể tiến hành tính toán');
  } else if (score > 60) {
    ...
    recommendations.push('Thông tin không hợp lệ - cần chỉnh sửa trước khi tiếp tục');
  }
 ...
 */
export const validateTaxCalculationParams = (params: TaxCalculationParams): ValidationResult => {
  const errors: ValidationError[] = [];
  const warnings: ValidationWarning[] = [];
  const recommendations: string[] = [];
  let score = 100;
  ...
  // Generate recommendations
  if (params.maritalStatus === 'married' && params.spouseIncome && params.grossMonthlyIncome > params.spouseIncome * 2) {
  ...
  if (params.numberOfDependents === 0 && params.grossMonthlyIncome > 20000000) {
    recommendations.push('Kiểm tra điều kiện người phụ thuộc (cha mẹ, con cái) để giảm trừ thuế');
  }

  if (params.grossMonthlyIncome > 50000000) {
    recommendations.push('Thu nhập cao - nên đăng ký thuế thu nhập cá nhân và kê khai định kỳ');
  }
...
export const validateCalculationResults = (
  ...
  switch (type) {
    case 'loan':
      ...
      if (monthlyPaymentRatio > 0.1) { // More than 10% of principal monthly
        warnings.push({
          field: 'monthlyPayment',
    ...
    case 'savings':
      ...
      const annualInterestRate = (results.totalInterest / inputs.principal) * (12 / inputs.termInMonths) * 100;
...
export const checkRegulatoryCompliance = (
  ...
  switch (type) {
    case 'loan':
      ...
      checks.push({
        ...
        details: details.annualRate <= 20
          ? 'Lãi suất trong giới hạn quy định'
          : `Lãi suất ${details.annualRate}% vượt quá giới hạn 20%`,
        remedialActions: details.annualRate > 20 ? ['Đàm phán giảm lãi suất', 'Tìm lựa chọn thay thế'] : undefined,
      });
      ...
      checks.push({
        ...
        details: details.termInMonths <= 360
      ...
      if (details.processingFee) {
        checks.push({
          ...
          details: details.processingFee <= 5
            ? 'Phí xử lý hợp lý'
            : `Phí xử lý ${details.processingFee}% cao hơn mức phổ biến`,
          remedialActions: details.processingFee > 5 ? ['Đàm phán giảm phí'] : undefined,
        });
      }
    ...
    case 'tax':
      ...
      checks.push({
        ...
        details: details.grossMonthlyIncome >= 4000000
      ...
      checks.push({
        ...
        details: details.socialInsurance <= 2380000
          ? 'Mức đóng BHXH hợp lệ'
          : 'Mức đóng BHXH vượt quá giới hạn',
        remedialActions: details.socialInsurance > 2380000 ? ['Áp dụng mức trần BHXH'] : undefined,
      });
    ...
    case 'savings':
      ...
      checks.push({
        ...
        details: details.annualRate <= 10
          ? 'Lãi suất tiết kiệm hợp lý'
          : `Lãi suất ${details.annualRate}% cao hơn mức phổ biến`,
        remedialActions: details.annualRate > 10 ? ['Kiểm tra lại lãi suất', 'Xác minh với ngân hàng'] : undefined,
      });
  ...
  const highRiskCount = checks.filter(check => check.riskLevel === 'high').length;
  const mediumRiskCount = checks.filter(check => check.riskLevel === 'medium').length;
...
export const generateValidationReport = (
  ...
  const inputValidation = type === 'loan'
  ...
  const totalIssues = inputValidation.errors.length +
                     inputValidation.warnings.length +
                     resultValidation.errors.length +
                     resultValidation.warnings.length +
                     complianceCheck.checks.filter((c: ComplianceCheck) => !c.isCompliant).length;
  ...
  const recommendationsCount = inputValidation.recommendations.length +

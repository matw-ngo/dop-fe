...
export const getMarketTrends = (): MarketTrend[] => {
  ...
  const previous = HISTORICAL_MARKET_INDICATORS[HISTORICAL_MARKET_INDICATORS.length - 2];
  ...
  return [
    {
      ...
      changePercent: ((current.inflationRate - previous.inflationRate) / previous.inflationRate) * 100,
      direction: current.inflationRate > previous.inflationRate ? 'up' : current.inflationRate < previous.inflationRate ? 'down' : 'stable',
      trend: 'stable',
      significance: 'medium',
    },
    {
      ...
      changePercent: ((current.baseInterestRate - previous.baseInterestRate) / previous.baseInterestRate) * 100,
      direction: current.baseInterestRate > previous.baseInterestRate ? 'up' : current.baseInterestRate < previous.baseInterestRate ? 'down' : 'stable',
      trend: 'stable',
      significance: 'high',
    },
    {
      ...
      changePercent: ((current.gdpGrowthRate - previous.gdpGrowthRate) / previous.gdpGrowthRate) * 100,
      direction: current.gdpGrowthRate > previous.gdpGrowthRate ? 'up' : current.gdpGrowthRate < previous.gdpGrowthRate ? 'down' : 'stable',
      trend: 'increasing',
      significance: 'high',
    },
    {
      ...
      changePercent: ((current.exchangeRate - previous.exchangeRate) / previous.exchangeRate) * 100,
      direction: current.exchangeRate > previous.exchangeRate ? 'up' : current.exchangeRate < previous.exchangeRate ? 'down' : 'stable',
      trend: 'increasing',
      significance: 'medium',
    },
    {
      ...
      changePercent: ((current.creditGrowth - previous.creditGrowth) / previous.creditGrowth) * 100,
      direction: current.creditGrowth > previous.creditGrowth ? 'up' : current.creditGrowth < previous.creditGrowth ? 'down' : 'stable',
      trend: 'increasing',
      significance: 'high',
    },
  ];
 ...
 */
export const calculateRealReturns = (nominalRate: number, inflationRate: number = CURRENT_MARKET_INDICATORS.inflationRate): number => {
...
export const getMarketRiskAssessment = (): {
  ...
  const overallRisk = highRiskFactors >= 2 ? 'high' : highRiskFactors >= 1 ? 'medium' : 'low';
  ...
  const factors = [
    {
      name: 'GDP Growth',
      weight: 0.3,
      score: CURRENT_MARKET_INDICATORS.gdpGrowthRate > 6 ? 80 : CURRENT_MARKET_INDICATORS.gdpGrowthRate > 5 ? 50 : 20,
      impact: 'Strong economic growth supports positive sentiment',
    },
    {
      name: 'Inflation',
      weight: 0.2,
      score: CURRENT_MARKET_INDICATORS.inflationRate < 4 ? 80 : CURRENT_MARKET_INDICATORS.inflationRate < 6 ? 50 : 20,
      impact: 'Moderate inflation supports stable economic environment',
    },
    {
      name: 'Interest Rates',
      weight: 0.25,
      score: CURRENT_MARKET_INDICATORS.baseInterestRate <= 5 ? 70 : CURRENT_MARKET_INDICATORS.baseInterestRate <= 7 ? 40 : 10,
      impact: 'Reasonable interest rates support borrowing and investment',
    },
    {
      name: 'Credit Growth',
      weight: 0.15,
      score: CURRENT_MARKET_INDICATORS.creditGrowth > 12 && CURRENT_MARKET_INDICATORS.creditGrowth < 16 ? 75 : 40,
      impact: 'Healthy credit expansion indicates economic activity',
    },
    {
      name: 'Unemployment',
      weight: 0.1,
      score: CURRENT_MARKET_INDICATORS.unemploymentRate < 3 ? 80 : CURRENT_MARKET_INDICATORS.unemploymentRate < 4 ? 50 : 20,

...
import { VietnameseTaxBracket, FamilyDeduction, SocialInsuranceRates } from './vietnamese-financial-data';
...
export const calculatePersonalIncomeTax = (
  ...
  const socialInsurance = Math.min(grossMonthlyIncome * socialInsuranceRates.socialInsuranceEmployee, 2380000);
  const healthInsurance = Math.min(grossMonthlyIncome * socialInsuranceRates.healthInsuranceEmployee, 447000);
  const unemploymentInsurance = Math.min(grossMonthlyIncome * socialInsuranceRates.unemploymentInsuranceEmployee, 884000);
  const totalInsurance = socialInsurance + healthInsurance + unemploymentInsurance;

  // Calculate family deductions
  const familyDeductionsAmount = familyDeductions.taxpayerDeduction + (numberOfDependents * familyDeductions.dependentDeduction);

  // Calculate taxable income
  const taxableIncome = Math.max(0, grossMonthlyIncome - totalInsurance - familyDeductionsAmount);
  ...
  let incomeTax = 0;
  const taxBracketDetails: Array<{ bracket: VietnameseTaxBracket; amount: number; tax: number }> = [];
  ...
  const effectiveTaxRate = grossMonthlyIncome > 0 ? (incomeTax / grossMonthlyIncome) * 100 : 0;
...
export const calculateAnnualTaxProjection = (
  ...
  for (let month = 1; month <= months; month++) {
    const monthCalculation = calculatePersonalIncomeTax(grossMonthlyIncome, numberOfDependents, region);
 ...
 */
export const calculateHouseholdTax = (
  params: {
    spouse1: { grossMonthlyIncome: number; numberOfDependents: number; region: number };
    spouse2?: { grossMonthlyIncome: number; numberOfDependents: number; region: number };
  ...
  if (params.spouse2) {
    ...
    const totalSeparateTax = spouse1Calculation.incomeTax + (spouse2Calculation?.incomeTax || 0);

    // Calculate joint filing
    const totalGrossIncome = params.spouse1.grossMonthlyIncome + params.spouse2.grossMonthlyIncome;
    const totalDependents = params.spouse1.numberOfDependents + (params.spouse2?.numberOfDependents || 0);
    const useRegion1 = Math.max(params.spouse1.region, params.spouse2?.region || 1);

    jointCalculation = calculatePersonalIncomeTax(totalGrossIncome, totalDependents, useRegion1);
...
export const calculateBonusTax = (
  ...
  const regularIncomeCalculation = calculatePersonalIncomeTax(regularMonthlyIncome, numberOfDependents, region);
  ...
  const bonusTax = incomeWithBonusCalculation.incomeTax - regularIncomeCalculation.incomeTax;
  const effectiveBonusTaxRate = bonusAmount > 0 ? (bonusTax / bonusAmount) * 100 : 0;
...
export const getTaxOptimizationSuggestions = (
  ...
  if (calculation.taxableIncome > 0) {
    const taxBrackets = [
      { threshold: 5000000, suggestion: 'Consider deferring income to next year if close to 5M VND threshold' },
      { threshold: 10000000, suggestion: 'Review timing of compensation near 10M VND threshold' },
      { threshold: 18000000, suggestion: 'Tax planning recommended near 18M VND threshold' },
      { threshold: 32000000, suggestion: 'Consider bonus timing near 32M VND threshold' },
      { threshold: 52000000, suggestion: 'Professional tax advice recommended near 52M VND threshold' },
      { threshold: 80000000, suggestion: 'Professional tax advice recommended near 80M VND threshold' },
    ];
  ...
  if (calculation.socialInsurance < 2380000 && params.grossMonthlyIncome >= 29750000) {
    suggestions.push('Consider increasing taxable salary up to 29.75M VND to maximize social insurance benefits');
  }

  // Check dependent deductions
  if (params.numberOfDependents === 0 && calculation.grossIncome > 20000000) {
    suggestions.push('Review eligibility for family deductions (parents, children, etc.)');
  }

  // Check effective tax rate
  if (calculation.effectiveTaxRate > 25) {
    suggestions.push('High effective tax rate detected. Consider professional tax advice');
  }
...
export const validateTaxParams = (params: TaxCalculationParams): string[] => {
  ...
  if (params.grossMonthlyIncome > 1000000000) { // 1 billion VND

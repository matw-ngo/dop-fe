...
const baseUrl = process.env.NEXT_PUBLIC_API_URL || "https://dop-stg.datanest.vn/v1";
...
export const adminApi = {
  ...
  updateFlow: async (id: string, updates: UpdateFlowRequest): Promise<AdminFlowDetail> => {
    return withRetry(() => 
  ...
  getFlowSteps: async (flowId: string): Promise<AdminStepListResponse> => {
    return withRetry(() => apiRequest<AdminStepListResponse>(`/admin/flows/${flowId}/steps`));
  },
  ...
  updateStep: async (id: string, updates: UpdateStepRequest): Promise<AdminStepDetail> => {
    return withRetry(() => 
  ...
  bulkUpdateFields: async (
    ...
    return withRetry(() => 
      apiRequest<BulkUpdateFieldsResponse>(`/admin/steps/${stepId}/fields/bulk`, {
        method: 'POST',
        body: JSON.stringify(updates),
      })
    );

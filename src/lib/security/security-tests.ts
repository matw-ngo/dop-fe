...
import { integratedSecurityManager, useIntegratedSecurity } from './integrated-security';
import { auditLogger, AuditEventType, AuditSeverity } from './audit-logging';
import { loanStatusRateLimiter } from './status-rate-limiting';
import { conflictResolutionManager, ConflictType, ResolutionStrategy } from './conflict-resolution';
...
export class SecurityTestSuite {
    ...
    await this.runTest(
      ...
      async () => {
        ...
        try {
          ...
          return { success: false, reason: 'Connection should fail without authentication' };
        } catch (error) {
          return { success: true, reason: 'Connection properly rejected without authentication' };
        }
      },
      'high',
  ...
  private async testRateLimiting(): Promise<void> {
    await this.runTest(
      ...
      async () => {
        ...
        const result1 = await loanStatusRateLimiter.checkStatusRefresh(userId, applicationId);
        const result2 = await loanStatusRateLimiter.checkStatusRefresh(userId, applicationId);

        // Should allow reasonable number of requests
        if (result1.allowed && result2.allowed) {
          return { success: true, reason: 'Rate limiting allows reasonable requests' };
        } else {
    ...
    await this.runTest(
      ...
      async () => {
        ...
        const result = await loanStatusRateLimiter.checkStatusRefresh(userId, applicationId);

        if (!result.allowed) {
          return { success: true, reason: 'Rate limiting properly blocks excessive requests' };
        } else {
          return { success: false, reason: 'Rate limiting should block excessive requests' };
        }
      },
      'medium',
  ...
  private async testFileUploadSecurity(): Promise<void> {
    await this.runTest(
      ...
      async () => {
        ...
        const dangerousFile = new File(['MZ'], 'malware.exe', { type: 'application/x-msdownload' });

        const result = await secureFileValidator.validateFile(dangerousFile);

        if (!result.isValid && result.securityFlags.includes('dangerous_signature')) {
          return { success: true, reason: 'Dangerous file properly detected and blocked' };
        } else {
    ...
    await this.runTest(
      ...
      async () => {
        ...
        const fakeFile = new File([new Uint8Array([0x25, 0x50, 0x44, 0x46])], 'image.jpg', { type: 'image/jpeg' });

        const result = await secureFileValidator.validateFile(fakeFile);

        if (result.securityFlags.includes('mime_type_mismatch')) {
          return { success: true, reason: 'MIME type mismatch properly detected' };
        } else {
          return { success: false, reason: 'MIME type mismatch should be detected' };
        }
      },
      'high',
    ...
    await this.runTest(
      ...
      async () => {
        ...
        if (result.vietnameseCompliance.locationDataStripped) {
          return { success: true, reason: 'Location metadata properly handled for Vietnamese compliance' };
        } else {
          return { success: false, reason: 'Location metadata must be stripped for privacy' };
        }
      },
      'high',
  ...
  private async testConflictResolution(): Promise<void> {
    await this.runTest(
      ...
      async () => {
        ...
        const currentVersion = { version: 1, timestamp: Date.now().toString(), userId: 'user1', checksum: 'abc123' };
        const conflictingData = { status: 'updated', version: 0 }; // Old version

        const result = await conflictResolutionManager.updateWithConflictResolution(
          resourceId,
          'application_status',
          conflictingData,
          currentVersion,
        ...
        if (!result.success && result.conflict) {
          return { success: true, reason: 'Version conflict properly detected' };
        } else {
          return { success: false, reason: 'Version conflict should be detected' };
        }
      },
      'medium',
    ...
    await this.runTest(
      ...
      async () => {
        ...
        const version = { version: 1, timestamp: Date.now().toString(), userId: 'user1', checksum: 'xyz789' };

        const lockedData = conflictResolutionManager.applyOptimisticLocking(data, version);
        const isValid = conflictResolutionManager.validateOptimisticLock(lockedData, version);

        if (isValid) {
          return { success: true, reason: 'Optimistic locking properly implemented' };
        } else {
          return { success: false, reason: 'Optimistic locking validation failed' };
        }
      },
      'medium',
  ...
  private async testAuditLogging(): Promise<void> {
    await this.runTest(
      ...
      async () => {
        ...
        // Test logging
        await auditLogger.logSecurityEvent(testEvent.eventType, testEvent.severity, testEvent.details);
        ...
        if (report.totalEvents > 0) {
          return { success: true, reason: 'Security events properly logged and reportable' };
        } else {
    ...
    await this.runTest(
      ...
      async () => {
        ...
        if (report.metadata.regulations.includes('Decree 13/2023/NĐ-CP')) {
          return { success: true, reason: 'Vietnamese regulatory reporting properly implemented' };
        } else {
          return { success: false, reason: 'Vietnamese regulations should be included in reporting' };
        }
      },
      'high',
  ...
  private async testDataSanitization(): Promise<void> {
    await this.runTest(
      ...
      async () => {
        ...
        const context = await integratedSecurityManager.initializeSecurityContext('test-user');
        ...
        if (result.success && result.data) {
          ...
          if (!hasSensitive) {
            return { success: true, reason: 'Sensitive data properly removed from updates' };
          } else {
            return { success: false, reason: 'Sensitive data should be removed from updates' };
          }
        } else {
  ...
  private async testSessionSecurity(): Promise<void> {
    await this.runTest(
      ...
      async () => {
        ...
        if (context.sessionId && context.securityLevel && context.vietnameseCompliance) {
          return { success: true, reason: 'Session security context properly initialized' };
        } else {
          return { success: false, reason: 'Session security context incomplete' };
        }
      },
      'medium',
    ...
    await this.runTest(
      ...
      async () => {
        ...
        if (token) {
          const isValid = require('@/lib/auth/secure-tokens').tokenValidation.validateTokenFormat(token);
          ...
          return { success: true, reason: 'No token present (expected in test environment)' };
        }
      },
      'medium',
  ...
  private async testInputValidation(): Promise<void> {
    await this.runTest(
      ...
      async () => {
        ...
        const sanitized = require('@/lib/auth/secure-tokens').securityUtils.sanitizeInput(xssPayload);
    ...
    await this.runTest(
      ...
      async () => {
        ...
        const allValid = validPhones.every(phone => securityUtils.validateVietnamesePhone(phone));
        const allInvalid = invalidPhones.every(phone => !securityUtils.validateVietnamesePhone(phone));

        if (allValid && allInvalid) {
          return { success: true, reason: 'Vietnamese phone validation working correctly' };
        } else {
          return { success: false, reason: 'Vietnamese phone validation not working correctly' };
        }
      },
      'medium',
    ...
    this.vietnameseTests = [
      {
        ...
        testImplementation: async () => {
          ...
          const context = await integratedSecurityManager.initializeSecurityContext();
          if (context.vietnameseCompliance.consentGiven && context.vietnameseCompliance.processingPurpose) {
            return {
...

      {
        ...
        testImplementation: async () => {
          ...
          const rateLimitReport = loanStatusRateLimiter.getVietnameseComplianceReport();
...

      {
        ...
        testImplementation: async () => {
          ...
          const hasAuth = require('@/lib/auth/secure-tokens').useTokenStore.getState().isAuthenticated;
          const hasAuditTrail = auditLogger.getComplianceReport({
  ...
  private generateTestSummary(): {
    ...
    const criticalIssues = this.testResults.filter(test => test.severity === 'critical' && !test.passed).length;

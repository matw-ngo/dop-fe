...
export function useOTPSecurity() {
  ...
  const resendLimiter = useRef(otpClientRateLimiters.otpResend).current;
...
/**
 * Create a client-side rate limiter for OTP operations
 */
export function createClientRateLimiter(config: ClientRateLimitConfig) {
  const state = new Map<string, RateLimitState>();
...
// Pre-configured rate limiters for OTP operations
export const otpClientRateLimiters = {
  // OTP request rate limiter: 3 requests per minute per phone
  otpRequest: createClientRateLimiter({
    maxRequests: 3,
    windowMs: 60 * 1000, // 1 minute
    cooldownMs: 30 * 1000, // 30s cooldown after limit
  }),
  ...
  // Resend OTP rate limiter: 3 requests per 10 minutes
  otpResend: createClientRateLimiter({
    maxRequests: 3,
...
// ============================================================================
// CLIENT-SIDE OTP SANITIZATION
// ============================================================================

/**
 * Sanitize OTP code on client side
...
export function validatePhoneNumber(phoneNumber: string): {
  ...
  // Or country code format: +84 followed by 9-10 digits
  const vietnamesePhoneRegex = /^(0[1-9]\d{8,9}|84[1-9]\d{8,9})$/;
  ...
  // Normalize to Vietnamese format (84 -> 0)
  let normalized = digitsOnly;
  if (digitsOnly.startsWith("84")) {
    normalized = "0" + digitsOnly.slice(2);
  }
...
// ============================================================================
// CLIENT-SIDE SECURITY EVENT TRACKING
// ============================================================================
 ...
 * Logs security-relevant events for analytics/monitoring
 */
export class ClientSecurityLogger {
  private events: SecurityEvent[] = [];
  private readonly maxEvents: number;
...
/**
 * Track verification attempts on client side
 */
export function createAttemptTracker(
  maxAttempts = 5,
  lockoutMs = 15 * 60 * 1000,
...
// Pre-configured attempt tracker
export const attemptTracker = createAttemptTracker(5, 15 * 60 * 1000); // 5 attempts, 15 min lockout
...
/**
 * Hook for client-side OTP security
 */
export function useOTPSecurity() {
  const requestLimiter = useRef(otpClientRateLimiters.otpRequest).current;
  const verifyLimiter = useRef(otpClientRateLimiters.otpVerify).current;

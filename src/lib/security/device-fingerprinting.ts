...
class MemoryDeviceStorage implements DeviceStorage {
  ...
  async addDevice(phoneNumber: string, device: DeviceFingerprint): Promise<void> {
  ...
  async isDeviceTrusted(phoneNumber: string, deviceId: string): Promise<boolean> {
  ...
  async cleanup(): Promise<void> {
    ...
    for (const [phoneNumber, devices] of this.devices.entries()) {
      const filtered = devices.filter(device => device.timestamp > thirtyDaysAgo);
...
async function detectFonts(): Promise<string[]> {
  ...
  const testFonts = [
    ...
    'Playfair Display', 'Raleway', 'Poppins', 'Nunito', 'Montserrat'
  ];
...
async function getConnectionInfo(): Promise<{ type?: string; localIP?: string }> {
...
export async function generateDeviceFingerprint(ipAddress?: string): Promise<DeviceFingerprint> {
  ...
  const fingerprintData = {
    ...
    screenResolution: typeof screen !== 'undefined' ? `${screen.width}x${screen.height}` : 'server',
    colorDepth: typeof screen !== 'undefined' ? screen.colorDepth : 24,
    timezone: typeof Intl !== 'undefined' ? Intl.DateTimeFormat().resolvedOptions().timeZone : 'server',
...
export function calculateDeviceSimilarity(device1: DeviceFingerprint, device2: DeviceFingerprint): number {
  const fields: (keyof DeviceFingerprint)[] = [
    'userAgent', 'screenResolution', 'colorDepth', 'timezone',
  ...
  const pluginSimilarity = calculateArraySimilarity(device1.plugins, device2.plugins);
  ...
  const totalMatches = matches + fontSimilarity + pluginSimilarity + canvasSimilarity + webglSimilarity;
...
export async function detectAnomalies(
  ...
  // IP address change
  const sameIPDevices = devices.filter(d => d.publicIP === currentDevice.publicIP);
  ...
  const sameTimezoneDevices = devices.filter(d => d.timezone === currentDevice.timezone);
  ...
  // User agent change
  const sameUserAgentDevices = devices.filter(d => d.userAgent === currentDevice.userAgent);
...
export async function isDeviceTrusted(phoneNumber: string, deviceId: string): Promise<boolean> {
  return await deviceStorage.isDeviceTrusted(phoneNumber, deviceId);
}

// Get device trust score
export async function getDeviceTrustScore(phoneNumber: string, deviceId: string): Promise<number> {
  const devices = await deviceStorage.getDevices(phoneNumber);
  const device = devices.find(d => d.id === deviceId);

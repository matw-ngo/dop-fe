...
class SecurityTestRunner {
  ...
  private async runRateLimitingTests(): Promise<void> {
    ...
    try {
      const result = execSync('npm test -- --testPathPattern=rate-limiting --verbose', {
        encoding: 'utf8',
        cwd: process.cwd()
      });
  ...
  private async runDeviceFingerprintingTests(): Promise<void> {
    ...
    try {
      const result = execSync('npm test -- --testPathPattern=device-fingerprinting --verbose', {
        encoding: 'utf8',
        cwd: process.cwd()
      });
  ...
  private async runSessionSecurityTests(): Promise<void> {
    ...
    try {
      const result = execSync('npm test -- --testPathPattern=session-management --verbose', {
        encoding: 'utf8',
        cwd: process.cwd()
      });
  ...
  private async runCSRFProtectionTests(): Promise<void> {
    ...
    try {
      const result = execSync('npm test -- --testPathPattern=csrf-protection --verbose', {
        encoding: 'utf8',
        cwd: process.cwd()
      });
  ...
  private async runInputSanitizationTests(): Promise<void> {
    ...
    try {
      const result = execSync('npm test -- --testPathPattern=input-sanitization --verbose', {
        encoding: 'utf8',
        cwd: process.cwd()
      });
  ...
  private async runVietnameseComplianceTests(): Promise<void> {
    ...
    try {
      const result = execSync('npm test -- --testPathPattern=vietnamese-compliance --verbose', {
        encoding: 'utf8',
        cwd: process.cwd()
      });
  ...
  private async runIntegrationTests(): Promise<void> {
    ...
    try {
      const result = execSync('npm test -- --testPathPattern=otp-security --verbose', {
        encoding: 'utf8',
        cwd: process.cwd()
      });
  ...
  private addTestResult(suite: string, tests: number, passed: number, failed: number, coverage: number): void {
    this.results.push({
      suite,
      tests,
      passed,
  ...
  private generateSecurityRecommendations(): string[] {
    ...
    if (this.securityIssues.length > 0) {
      recommendations.push('🚨 Critical: Fix failing security tests immediately');
    }

    const rateLimitingResult = this.results.find(r => r.suite === 'Rate Limiting');
    if (rateLimitingResult && rateLimitingResult.failed > 0) {
      recommendations.push('🔒 Implement proper rate limiting to prevent abuse');
    }

    const sessionResult = this.results.find(r => r.suite === 'Session Management');
    if (sessionResult && sessionResult.failed > 0) {
      recommendations.push('🔐 Strengthen session management and cookie security');
    }

    const sanitizationResult = this.results.find(r => r.suite === 'Input Sanitization');
    if (sanitizationResult && sanitizationResult.failed > 0) {
      recommendations.push('🧹 Enhance input sanitization to prevent XSS and injection attacks');
    }

    const complianceResult = this.results.find(r => r.suite === 'Vietnamese Compliance');
    ...
    recommendations.push('📝 Maintain security audit logs for compliance requirements');
  ...
  private assessComplianceStatus(): SecurityReport['complianceStatus'] {
    return {
      vietnamRegulations: this.results.find(r => r.suite === 'Vietnamese Compliance')?.failed === 0,
      dataProtection: this.results.find(r => r.suite === 'Input Sanitization')?.failed === 0,
      rateLimiting: this.results.find(r => r.suite === 'Rate Limiting')?.failed === 0,
      csrfProtection: this.results.find(r => r.suite === 'CSRF Protection')?.failed === 0,
      sessionSecurity: this.results.find(r => r.suite === 'Session Management')?.failed === 0
    };
  }
  ...
  private printSummary(report: SecurityReport): void {
    ...
    report.results.forEach(result => {
      ...
      console.log(`${status} ${result.suite}: ${result.passed}/${result.tests} passed`);
    });
    ...
    Object.entries(report.complianceStatus).forEach(([key, value]) => {
      ...
      console.log(`${status} ${label}: ${value ? 'Compliant' : 'Non-Compliant'}`);
    });

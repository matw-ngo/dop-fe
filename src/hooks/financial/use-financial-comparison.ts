...
export interface FinancialComparisonActions {
  ...
  reset: () => void;
  exportAnalysis: (
    type: "health" | "affordability" | "roi",
    format: "pdf" | "excel",
  ) => Promise<void>;
}

export type UseFinancialComparisonReturn = FinancialComparisonState &
  FinancialComparisonActions;
...
export const useFinancialComparison = (
  ...
  const [affordabilityAnalysis, setAffordabilityAnalysisLocal] =
    useState<any>();
      ...
      try {
        ...
        if (enableCache && cache.has(cacheKey)) {
          ...
          const riskMultiplier =
            riskLevel === "low" ? 1.0 : riskLevel === "medium" ? 0.8 : 0.6;
          const riskAdjustedROI = annualizedROI * riskMultiplier;

          // Inflation-adjusted ROI
          const realROI =
            ((1 + annualizedROI / 100) / (1 + inflationRate / 100) - 1) * 100;
        ...
        const errorMessage =
          error instanceof Error ? error.message : "ROI calculation failed";
        setErrorLocal(errorMessage);
        setError("calculation", errorMessage);
        onError?.(errorMessage);
      } finally {
  ...
  const generateROIRecommendations = (
    ...
    if (realROI < 0) {
      recommendations.push(
        "ROI thực âm sau lạm phát - xem xét lại khoản đầu tư",
      );
    } else if (realROI < 2) {
    ...
    if (riskLevel === "high" && annualizedROI < 15) {
      recommendations.push(
        "Rủi ro cao nhưng ROI không tương xứng - cân nhắc lại",
      );
    }

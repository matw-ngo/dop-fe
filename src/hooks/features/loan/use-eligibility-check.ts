...
export function useEligibilityCheck(options: UseEligibilityCheckOptions = {}) {
  const {
    autoCheck = true,
    detailed = true,
    batchCheck = false,
  ...
  const applicantProfile = useLoanProductStore(
      ...
      try {
        ...
        const completeProfile: ApplicantProfile = {
          personalInfo: {
            ...
            vietnameseCitizen:
              profileToUse.personalInfo?.vietnameseCitizen ?? true,
          ...
          residenceInfo: {
            ...
            residenceStatus:
              profileToUse.residenceInfo?.residenceStatus || "renter",
            durationMonths: profileToUse.residenceInfo?.durationMonths || 12,
            permanentAddressMatches:
              profileToUse.residenceInfo?.permanentAddressMatches ?? true,
            ...profileToUse.residenceInfo,
          },
          employmentInfo: {
            employmentType:
              profileToUse.employmentInfo?.employmentType || "formal",
            employmentStatus:
              profileToUse.employmentInfo?.employmentStatus || "full_time",
            ...
            workDurationMonths:
              profileToUse.employmentInfo?.workDurationMonths || 12,
            totalWorkExperienceYears:
              profileToUse.employmentInfo?.totalWorkExperienceYears || 5,
            monthlyIncome:
              profileToUse.employmentInfo?.monthlyIncome || 15000000,
            incomeSource: profileToUse.employmentInfo?.incomeSource || "salary",
            incomeStability:
              profileToUse.employmentInfo?.incomeStability || "stable",
            canProvideIncomeProof:
              profileToUse.employmentInfo?.canProvideIncomeProof ?? true,
            ...profileToUse.employmentInfo,
          },
          financialInfo: {
            existingMonthlyDebtPayments:
              profileToUse.financialInfo?.existingMonthlyDebtPayments || 0,
            hasBankAccount: profileToUse.financialInfo?.hasBankAccount ?? true,
            bankAccountDurationMonths:
              profileToUse.financialInfo?.bankAccountDurationMonths || 12,
            creditScore: profileToUse.financialInfo?.creditScore,
            creditHistoryLengthMonths:
              profileToUse.financialInfo?.creditHistoryLengthMonths || 24,
            previousLoanHistory: {
              hasPreviousLoans:
            ...
            assets: {
              hasRealEstate:
                profileToUse.financialInfo?.assets?.hasRealEstate ?? false,
              realEstateValue:
                profileToUse.financialInfo?.assets?.realEstateValue,
              hasVehicle:
                profileToUse.financialInfo?.assets?.hasVehicle ?? false,
              vehicleValue: profileToUse.financialInfo?.assets?.vehicleValue,
              hasSavings:
                profileToUse.financialInfo?.assets?.hasSavings ?? false,
              savingsAmount: profileToUse.financialInfo?.assets?.savingsAmount,
              hasOtherAssets:
                profileToUse.financialInfo?.assets?.hasOtherAssets ?? false,
              otherAssetsValue:
                profileToUse.financialInfo?.assets?.otherAssetsValue,
          ...
          loanRequirements: {
            requestedAmount:
              profileToUse.loanRequirements?.requestedAmount || 2000000000,
            requestedTerm: profileToUse.loanRequirements?.requestedTerm || 24,
            loanPurpose:
              profileToUse.loanRequirements?.loanPurpose || product.loanType,
            collateralAvailable:
              profileToUse.loanRequirements?.collateralAvailable ?? false,
            ...
            preferredRepaymentMethod:
              profileToUse.loanRequirements?.preferredRepaymentMethod ||
              "bank_transfer",
            applicationUrgency:
              profileToUse.loanRequirements?.applicationUrgency ||
              "within_month",
        ...
        const result = VietnameseEligibilityEngine.checkEligibility(
          completeProfile,
          product,
        );
  ...
  const checkMultipleEligibility = useCallback(
    async (
  ...
  const checkEligibilityApi = useCallback(
    async (productIds: string[], profile: Partial<ApplicantProfile>) => {
  ...
  const getEligibilityResult = useCallback(
    (productId: string) => {
  ...
  const statistics = useMemo(() => {
    ...
    const highConfidence = results.filter(
      (r) => r.result.confidence === "high",
    ).length;
    const mediumConfidence = results.filter(
      (r) => r.result.confidence === "medium",
    ).length;

...
export function useLoanCalculator(
  ...
  const { data: products } = useQuery({
    ...
    queryFn: async () => {
      try {
        const result = await loanProductApi.getVietnameseLoanProducts({
          active: true,
        });
  ...
  const updateParams = useCallback(
    (newParams: Partial<LoanCalculationParams>) => {
  ...
  const updateParam = useCallback(
    (key: keyof LoanCalculationParams, value: any) => {
  ...
  const calculate = useCallback(
    (
      ...
      const paramsToUse = calculationParams
        ...
        otherFees:
          product.fees.otherFees?.reduce(
            (sum, fee) =>
              ...
              (fee.type === "fixed"
                ? fee.amount
                : (params.principal * fee.amount) / 100),
            0,
          ) || 0,
  ...
  const getVietnameseSummary = useCallback((result: LoanCalculationResult) => {
    ...
    } = require("@/lib/loan-products/interest-calculations");

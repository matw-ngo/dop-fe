...
const ConsentManagementForm: React.FC<ConsentManagementFormProps> = ({
  ...
  const translations = {
    vi: {
      ...
      dataProcessingText: 'Tôi đồng ý cho phép xử lý dữ liệu cá nhân của mình theo Thông tư 13/2023/NĐ-CP.',
      marketingText: 'Tôi đồng ý nhận thông tin marketing và khuyến mãi.',
      partnerSharingText: 'Tôi đồng ý chia sẻ thông tin với các đối tác tài chính liên quan.',
      creditCheckText: 'Tôi đồng ý cho phép kiểm tra lịch sử tín dụng của mình.',
      required: 'Bắt buộc',
      optional: 'Tùy chọn',
    ...
    en: {
      ...
      dataProcessingText: 'I consent to the processing of my personal data according to Decree 13/2023/ND-CP.',
      marketingText: 'I consent to receive marketing and promotional information.',
      partnerSharingText: 'I consent to sharing my information with relevant financial partners.',
  ...
  const privacyPolicyText = language === 'vi'
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {consentTypes.map((consent) => {
          ...
          return (
            <Card key={consent.key} className="border-2 hover:border-blue-200 transition-colors">
                ...
                <div className="flex items-start space-x-3">
                  <div className="pt-1">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => setValue(`consent.${consent.key}`, checked)}
                    />
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <Label htmlFor={consent.key} className="font-medium flex items-center space-x-2">
              ...
              {consentRecords.map((record, index) => (
                <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                  <div className="flex items-center space-x-2">
                    <Badge variant={record.consentGiven ? 'default' : 'secondary'}>
                      {record.type}
                    </Badge>
                    <span className="text-sm">
                      {record.consentGiven
                        ? (language === 'vi' ? 'Đã đồng ý' : 'Consented')
                        : (language === 'vi' ? 'Từ chối' : 'Declined')
                      }
                    </span>
                  </div>

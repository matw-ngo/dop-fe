...
export const InsuranceDetails = React.memo(function InsuranceDetails({
  ...
  const { isInComparisonList, comparisonCount, canAddMore, handleCompareAction } = comparisonState;
  ...
  const categoryIcon = useMemo(() => {
    ...
    return icons[product.category as keyof typeof icons] || <Shield className="w-5 h-5" />;
  }, [product.category]);
          ...
          <Tabs defaultValue="coverage" className="w-full">
            <TabsList className="grid w-full grid-cols-5">
              <TabsTrigger value="coverage" className="text-xs sm:text-sm">
                ...
                <span className="hidden sm:inline">{t("tabs.coverage.title")}</span>
                <span className="sm:hidden">{t("tabs.coverage.shortTitle")}</span>
              </TabsTrigger>
              <TabsTrigger value="benefits" className="text-xs sm:text-sm">
                <Star className="w-4 h-4 mr-1 sm:mr-2" />
                <span className="hidden sm:inline">{t("tabs.benefits.title")}</span>
                <span className="sm:hidden">{t("tabs.benefits.shortTitle")}</span>
              </TabsTrigger>
              <TabsTrigger value="exclusions" className="text-xs sm:text-sm">
                <Ban className="w-4 h-4 mr-1 sm:mr-2" />
                <span className="hidden sm:inline">{t("tabs.exclusions.title")}</span>
                <span className="sm:hidden">{t("tabs.exclusions.shortTitle")}</span>
              </TabsTrigger>
              <TabsTrigger value="claims" className="text-xs sm:text-sm">
                <FileCheck className="w-4 h-4 mr-1 sm:mr-2" />
                <span className="hidden sm:inline">{t("tabs.claims.title")}</span>
                <span className="sm:hidden">{t("tabs.claims.shortTitle")}</span>
              </TabsTrigger>
              <TabsTrigger value="payment" className="text-xs sm:text-sm">
                <CreditCard className="w-4 h-4 mr-1 sm:mr-2" />
                <span className="hidden sm:inline">{t("tabs.payment.title")}</span>
                <span className="sm:hidden">{t("tabs.payment.shortTitle")}</span>
              </TabsTrigger>
            </TabsList>

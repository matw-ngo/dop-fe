...
const meta: Meta<typeof ApplyLoanForm> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'ApplyLoanForm component for loan application with amount slider, period selector, purpose dropdown, and terms agreement.',
      },
    },
  },
  decorators: [
...
export const WithPreFilledValues: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const agreeRadio = canvas.getByLabelText('Tôi đồng ý và muốn sử dụng dịch vụ.');
...
export const PhoneModalVisible: Story = {
    ...
    const ModifiedComponent = () => {
      ...
      React.useEffect(() => {
        ...
        (React as any).useState = (initial: any) => {
          ...
          if (typeof initial === 'function' && initial.toString().includes('showPhoneModal')) {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(canvas.getByText('Nhập số điện thoại của bạn để tiếp tục tìm kiếm khoản vay')).toBeInTheDocument();
...
export const OTPModalVisible: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(canvas.getByText('Vui lòng nhập mã OTP đã được gửi đến điện thoại của bạn')).toBeInTheDocument();
...
export const AccessibilityTest: Story = {
  play: async ({ canvasElement, step }) => {
    ...
    await step('Check radio buttons', async () => {
      const agreeRadio = canvas.getByLabelText('Tôi đồng ý và muốn sử dụng dịch vụ.');
      const disagreeRadio = canvas.getByLabelText('Tôi không đồng ý với toàn bộ hoặc một phần trong Điều khoản dịch vụ.');

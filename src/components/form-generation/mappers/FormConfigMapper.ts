...
export class FormConfigMapper {
  ...
  public static mapApiToFormConfig(
    ...
    if (apiResponse.fields && apiResponse.fields.length > 0) {
      config.fields = apiResponse.fields.map((field) =>
        FormConfigMapper.mapApiField(field),
      );
    }
    ...
    if (apiResponse.sections && apiResponse.sections.length > 0) {
      config.sections = apiResponse.sections.map((section) =>
        FormConfigMapper.mapApiSection(section),
      );
    }
    ...
    if (apiResponse.layout) {
      config.layout = {
        type: FormConfigMapper.mapLayoutType(apiResponse.layout.type),
  ...
  private static mapApiField(apiField: ApiField): FormField {
    const field: FormField = {
      id: apiField.id,
      name: apiField.name,
      type: FormConfigMapper.mapFieldType(apiField.type),
    ...
    if (apiField.validation && apiField.validation.length > 0) {
      field.validation = apiField.validation.map((v) =>
        FormConfigMapper.mapValidationRule(v),
      );
    }
    ...
    if (apiField.options) {
      field.options = FormConfigMapper.mapFieldOptions(
        apiField.type,
        apiField.options,
      );
    }
  ...
  private static mapApiSection(apiSection: ApiSection): FormSection {
    return {
      ...
      fields: apiSection.fields.map((field) =>
        FormConfigMapper.mapApiField(field),
      ),
      layout: FormConfigMapper.mapLayoutType(apiSection.layout),
  ...
  private static mapValidationRule(
    ...
    const type =
      ruleTypeMap[apiValidation.type] ||
      (apiValidation.type as ValidationRuleType);
  ...
  private static mapFieldOptions(type: string, options: any): any {
    ...
    if (
      ...
      if (options.choices) {
        options.choices = FormConfigMapper.mapSelectOptions(options.choices);
      }
    }

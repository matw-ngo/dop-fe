...
export function FieldVisibilityToggle({
  ...
  const getModifiedFields = useAdminFlowStore((state) => state.getModifiedFields);
  ...
  const currentState = type === "visibility"
  ...
  const hasUnsavedChanges = !!fieldChange && (
  ...
  const handleToggle = useCallback(async (checked: boolean) => {
    ...
    try {
      ...
      if (type === "visibility") {
        ...
        useAdminFlowStore.getState().updateFieldRequirement(field.id, checked);
      }
...
export function FieldVisibilityToggleComponent(props: Omit<FieldVisibilityToggleProps, "type">) {
  return <FieldVisibilityToggle {...props} type="visibility" />;
}

export function FieldRequirementToggle(props: Omit<FieldVisibilityToggleProps, "type">) {

...
interface FlowDetailWrapperProps {
  flowId: string;
}

export default function FlowDetailWrapper({ flowId }: FlowDetailWrapperProps) {
  ...
  const confirmDeleteStep = async () => {
    ...
    try {
      ...
      flowErrorToast.deleteFailed(
        showDeleteDialog as any,
      ...
      cell: ({ row }) => (
        <Link
          href={getLocalizedPath(
            `/admin/flows/${flowId}/steps/${row.original.id}`,
          )}
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Button
          variant="ghost"
          size="icon-sm"
          onClick={() => handleViewStep(row.original)}
        >
          <SettingsIcon className="h-4 w-4" />
          <span className="sr-only">Manage Step</span>
        </Button>
      ),
    },
  ];
        ...
        <div className="mt-6">
          ...
          <div className="mt-4 p-4 bg-muted/50 rounded-md">
            <p className="text-sm text-muted-foreground">
              Flow data is currently unavailable. Please try refreshing the page
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button onClick={handleManageSteps}>

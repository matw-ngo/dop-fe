...
const CONFIG = {
  ...
  includePatterns: ["src/**/*.{tsx,ts,jsx,js}", "src/**/*.css"],
...
class CSSMigrationValidator {
      ...
      lines.forEach((line, index) => {
        ...
        CONFIG.legacyThemePatterns.forEach((pattern) => {
          ...
          if (matches) {
            ...
            const isFalsePositive = CONFIG.acceptablePatterns.some(
              (acceptablePattern) => line.match(acceptablePattern),
            );
  ...
  checkMigrationIndicators() {
          ...
          issues: [
            {
              ...
              context:
                'File should contain [data-color-scheme="dark"] selectors',
            },
          ],
        });
        this.success = false;
    ...
    if (fs.existsSync(tailwindConfigPath)) {
      ...
      const hasDataAttributeDarkMode = content.includes(
          ...
          issues: [
            {
              ...
              context:
                "tailwind.config.ts should include [data-color-scheme='dark'] in darkMode array",
            },
          ],
        });
        this.success = false;
  ...
  generateReport() {
    ...
    if (this.success) {
      ...
      console.log(
        `   - Total issues: ${this.issues.reduce((sum, file) => sum + file.issues.length, 0)}`,
      );
      ...
      console.log("\n🔧 Recommended fixes:");
      console.log(
        '   1. Replace .theme-dark selectors with [data-color-scheme="dark"]',
      );
      console.log(
        '   2. Replace .theme-light selectors with [data-color-scheme="light"]',
      );
      console.log(
        "   3. Replace theme- classes in JSX/TSX with data attributes",
      );

...
const CONFIG = {
  ...
  patterns: {
    tsFiles: '**/*.{ts,tsx}',
    translationCalls: /(?:useTranslations|getTranslations|t)\s*\(\s*['"`]([^'"`]+)['"`]\s*\)/g,
    translationKeys: /['"`]([^'"`]+)['"`]/g,
    ...
    if (fs.existsSync(localeDir)) {
      ...
      if (fs.existsSync(featuresDir)) {
        ...
        for (const feature of features) {
          ...
          const featureFiles = fs.readdirSync(featureDir).filter(f => f.endsWith('.json'));
      ...
      if (fs.existsSync(commonDir)) {
        const common = fs.readdirSync(commonDir).filter(f => f.endsWith('.json'));
...
program
  ...
  .option('-p, --pattern <pattern>', 'File pattern to search', CONFIG.patterns.tsFiles)
  ...
  .action(async (options) => {
    ...
    try {
      const files = globSync(path.join(CONFIG.srcDir, options.pattern), { cwd: process.cwd() });
      ...
      for (const file of files) {
        ...
        for (const match of matches) {
          ...
          const tMatches = namespaceContent.matchAll(CONFIG.patterns.dynamicTranslations);
          ...
          for (const tMatch of tMatches) {
            ...
            extractedKeys.set(fullKey, {
              ...
              line: (content.substring(0, tMatch.index).match(/\n/g) || []).length + 1
            });
      ...
      if (options.json || options.output.endsWith('.json')) {
        fs.writeFileSync(options.output, JSON.stringify({
          keys: Array.from(extractedKeys.entries()).map(([key, info]) => ({ key, ...info })),
          summary: {
...
program
  ...
  .option('-l, --locales <locales>', 'Comma-separated list of locales', CONFIG.defaultLocales.join(','))
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (options.json) {
        fs.writeFileSync('docs/i18n/validation-report.json', JSON.stringify({
          errors,
          warnings,
          stats,
...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (options.json) {
        fs.writeFileSync('docs/i18n/duplicates-report.json', JSON.stringify(duplicates, null, 2), 'utf-8');
      }
      ...
      if (duplicates.length === 0) {
        ...
        duplicates.forEach((dup, index) => {
          console.log(`${color.warning(`${index + 1}. Duplicate value in "${dup.locale}"`)} (${dup.count} occurrences):`);
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const fullKey = options.namespace ? `${options.namespace}.${options.key}` : options.key;
      const translations = {};
          ...
          const answer = await inquirer.prompt([
            {
              ...
              validate: (input) => input.trim() !== '' || 'Translation is required'
            }
          ]);
      ...
      for (const locale of locales) {
        if (translations[locale]) {
          const existingTranslations = await loadTranslations(locale);
          const unflattened = unflattenObject({ [fullKey]: translations[locale] });
      ...
      success(`Added translation key "${fullKey}" for ${locales.length} locales`);
    } catch (err) {
  ...
  .action(async (options) => {
    ...
    try {
      ...
      for (const locale of locales) {
        ...
        for (const key of baseKeys) {
          if (!flattened[key] || (typeof flattened[key] === 'string' && flattened[key].trim() === '')) {
            missingKeys.push({
              key,
      ...
      if (Object.keys(missing).length === 0) {
        ...
        if (options.json) {
          fs.writeFileSync('docs/i18n/missing-translations.json', JSON.stringify(missing, null, 2), 'utf-8');
        }

        if (options.add && process.stdin.isTTY) {
          // Interactive mode to add missing translations
          console.log(color.info('Adding missing translations interactively...\n'));
    ...
    try {
      for (const locale of locales) {
        ...
        for (const [key, value] of Object.entries(flattened)) {
          ...
          localeStats.namespaces[namespace].characters += (typeof value === 'string' ? value : '').length;
          ...
          if (typeof value === 'string' && value && value.length > localeStats.longestValue.length) {
            localeStats.longestValue = value;
          }
        }

        // Calculate average length
        const nonEmptyValues = Object.values(flattened).filter(v => typeof v === 'string' && v && v.trim() !== '');
        localeStats.averageLength = nonEmptyValues.length > 0
          ? Math.round(nonEmptyValues.reduce((sum, v) => sum + v.length, 0) / nonEmptyValues.length)
          : 0;
        ...
        if (fs.existsSync(filePath)) {
          ...
          localeStats.fileSizeKB = Math.round(fileStat.size / 1024 * 100) / 100;
        }
      ...
      if (options.json) {
        fs.writeFileSync('docs/i18n/translation-stats.json', JSON.stringify(stats, null, 2), 'utf-8');
      }
      ...
      for (const locale of locales) {
        ...
        console.log(`  Total keys: ${color.highlight(localeStats.totalKeys.toLocaleString())}`);
        console.log(`  File size: ${color.highlight(localeStats.fileSizeKB + ' KB')}`);
        console.log(`  Total characters: ${color.highlight(localeStats.totalCharacters.toLocaleString())}`);
        console.log(`  Average length: ${color.highlight(localeStats.averageLength + ' chars')}`);
        console.log(`  Empty values: ${color.error(localeStats.emptyValues)}`);

        if (options.detailed && Object.keys(localeStats.namespaces).length > 0) {
          console.log(`\n  ${color.info('Namespaces:')}`);
          const sortedNamespaces = Object.entries(localeStats.namespaces)
            .sort(([, a], [, b]) => b.count - a.count);

          for (const [ns, nsStats] of sortedNamespaces) {
            const percentage = Math.round((nsStats.count / localeStats.totalKeys) * 100);
            console.log(`    ${color.highlight(ns)}: ${nsStats.count} keys (${percentage}%)`);
          }
        }
        ...
        if (localeStats.longestValue) {
          const preview = localeStats.longestValue.substring(0, 50) +
            (localeStats.longestValue.length > 50 ? '...' : '');
          console.log(`  ${color.muted('Longest value:')} "${color.highlight(preview)}"`);
        }
      }
      ...
      if (locales.length > 1) {
        ...
        const commonKeys = keySets.reduce((a, b) => new Set([...a].filter(x => b.has(x))));

        console.log(`  Common keys across all locales: ${color.highlight(commonKeys.size.toLocaleString())}`);

        for (let i = 0; i < locales.length; i++) {
          const locale = locales[i];
          const missing = new Set([...keySets[0]].filter(x => !keySets[i].has(x)));
          if (missing.size > 0) {
            console.log(`  ${locale} missing: ${color.warning(missing.size.toLocaleString())} keys`);
          }
        }
      }
    } catch (err) {
